...
impl TraderClient {
    ...
    pub fn new_by_wallet_id(wallet_id: u8) -> Result<Self, TradingError> {
        ...
        let position_storage = PositionStorage::new("positions.db")?;
        let strategy_storage = StrategyStorage::new("positions.db")?;
    ...
    pub async fn farm_points_on_backpack_from_multiple_wallets(
        ...
        // Save strategy first
        let strategy_storage = StrategyStorage::new("positions.db")?;
        strategy_storage.save_strategy(&strategy)?;
        
        // Save all positions
        let position_storage = PositionStorage::new("positions.db")?;
        for position in strategy.longs.iter().chain(strategy.shorts.iter()) {
            position_storage.save_position(position)?;
        }
    ...
    pub fn update_position_status(
        ...
        self.position_storage.update_position_status(id, status, closed_at, realized_pnl)
    }

    /// Delete a position from storage (use sparingly)
    pub fn delete_position(&self, id: &str) -> Result<(), TradingError> {
        self.position_storage.delete_position(id)
    }
    ...
    pub fn save_strategy(&self, strategy: &TradingStrategy) -> Result<(), TradingError> {
        self.strategy_storage.save_strategy(strategy)
    }
    ...
    pub fn get_all_strategies(&self) -> Result<Vec<StrategyMetadata>, TradingError> {
        self.strategy_storage.get_all_strategies()
    }
}